...
class File {
 ...
 private:
  ...
  tsl::hopscotch_map<std::string, std::shared_ptr<Weights>> _weights{};
 ...
 public:
  ...
  std::shared_ptr<Weights> read_weights(std::string_view name);
  std::shared_ptr<Weights> read_weights(std::string_view name, Weights::Type type);
