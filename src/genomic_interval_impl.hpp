...
inline GenomicInterval GenomicInterval::parse_ucsc(const ChromosomeSet &chroms, std::string query) {
  if (query.empty()) {
...
inline GenomicInterval GenomicInterval::parse_bed(const ChromosomeSet &chroms,
...
inline std::size_t std::hash<coolerpp::GenomicInterval>::operator()(
    const coolerpp::GenomicInterval &gi) const {
  return coolerpp::internal::hash_combine(0, gi.chrom(), gi.start(), gi.end());
}

constexpr fmt::format_parse_context::iterator fmt::formatter<coolerpp::GenomicInterval>::parse(
...
inline fmt::format_context::iterator fmt::formatter<coolerpp::GenomicInterval>::format(
    const coolerpp::GenomicInterval &gi, format_context &ctx) const {
